...
export async function getDoctorsData(): Promise<Doctor[]> {
  try {
    // Try fetching from our FastAPI backend first (for enriched data)
    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || "http://127.0.0.1:3000";
    const apiResponse = await fetch(`${baseUrl}/api/companies`, { next: { revalidate: 0 } }).catch(() => null);
    
    if (apiResponse && apiResponse.ok) {
      const apiData = await apiResponse.json();
      if (apiData && apiData.length > 0) {
        return apiData as Doctor[];
      }
    }

    // Fallback to direct GSheet CSV if API is unavailable or empty
